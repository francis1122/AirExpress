...
public class PolyMesh : MonoBehaviour
{
	...
	void UpdateCollider(List<Vector3> points, int[] tris)
	{
		...
		if (meshCollider != null)
		{
			...
			//convert 3d mesh to 2d polygon collider points
			var polyPoints = meshCollider.points;
			if(polyPoints == null){
			//	polyPoints = new Vector2[];
			}
			//polyPoints.RemoveAt(1);
			var test = points;
			var vect = new Vector2[test.Count];
			var abb = 0;
			foreach(Vector3 s in test)
			{
				vect[abb] = new Vector2(s.x, s.y);
				abb++;
			}
			//meshCollider.points = vect;
			//meshCollider.pathCount = vect.Length;
			meshCollider.SetPath(0, vect);

		//	var mesh.vertices = vertices.ToArray();
			/*
			//Find the mesh (create it if it doesn't exist)
